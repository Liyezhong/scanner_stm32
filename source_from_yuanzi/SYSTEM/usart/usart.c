...
{	//PD2,PC12
    ...
    HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
		...
		__HAL_UART_ENABLE_IT(&huart5,UART_IT_RXNE);		//开启接收中断
		
}

void usart5_sendchar(uint8_t ch)
{ 	
	while((UART5->SR&0X40)==0);//循环发送,直到发送完毕   
	UART5->DR = (u8) ch;      
}

void usart5_sendstring(uint8_t *cmd,int length)
{ 	
	int i=0;
	for(i=0;i<length;i++) usart5_sendchar(cmd[i]);
}
