...
//    HAL_NVIC_SetPriority(DCMI_IRQn,1,1); //set to 1 will lead jpeg down part extra part. 
...
//	  DCMI->CR|=DCMI_CR_ENABLE; //调了老子一个下午，还是要看寄存器.关键时候;
...
{ 
    DCMI->CR&=~(DCMI_CR_CAPTURE);       //关闭捕获
	  delay_ms(100);
//    while(DCMI->CR&0X01);               //等待传输完成,不知为何全速跑的时候会卡在这里;
...
extern uint8_t photo_QRcode_mode;
uint32_t focus_show_counter = 10;//use to show focus effect time.
uint8_t taking_picture_counter = 0;
...
//	  printf("frame:%d.\n",frame++);	//debug.似乎这一句会影响接收效率; 会提示接收到的JPEG数据不完整;中断里面做事尽量简洁;
//  	LCD_Draw_Circle(160,120,20);
//	  focus_show_counter ++ ;

//		GUI_SetColor(GUI_GREEN);
//		GUI_DrawRoundedFrame(80,60,240,180,5,5);
	
//	printf("frame irq: %d.\n",taking_picture_counter);
//	if(taking_picture_counter==2)
//	{
//		taking_picture_counter = 0;
	  jpeg_data_process();//jpeg数据处理
	
    __HAL_DCMI_CLEAR_FLAG(&DCMI_Handler,DCMI_FLAG_FRAMERI);//清除中断标志位;自己加;
